...
class tmvc
{
  ...
  // the parameters of the action of the URL
  var $action_params = null;
  /**
   * class constructor
   *
   * @access	public
  ...
  public function main()
  {
    ...
    $this->controller->{$this->action}($this->action_params);
  ...
  public function setupAction()
  {
    if(!empty($this->config['root_action'])) {  
      ...
      if(count($this->url_segments) >= 3){
          for($counter = 3; $counter <= count($this->url_segments); $counter++) {

            $this->action_params .= $this->url_segments[$counter];

            if($counter != count($this->url_segments)){
              $this->action_params .= ',';
            }
            $counter++;
        }// end param setup
